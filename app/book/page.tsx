...
function PhotoItem({ photo, rotation }: { photo: Photo; rotation: number }) {
  const [orientation, setOrientation] = useState<PhotoOrientation>("square");

  const imageClass =
    orientation === "landscape"
      ? "max-h-[20vh] max-w-[92%] sm:max-h-[23vh] sm:max-w-[92%]"
      : orientation === "portrait"
        ? "max-h-[25vh] max-w-[68%] sm:max-h-[28vh] sm:max-w-[68%]"
        : "max-h-[23vh] max-w-[78%] sm:max-h-[26vh] sm:max-w-[78%]";

  return (
    <motion.figure
      initial={{ opacity: 0, scale: 0.92, rotate: rotation }}
      animate={{ opacity: 1, scale: 1, rotate: rotation }}
      className="relative flex min-h-0 w-fit max-w-full justify-self-center items-center justify-center bg-[#fffdf8] p-0.5 shadow-[0_4px_10px_-6px_rgba(51,64,77,.4)] sm:p-1"
    >
      <div className="flex max-h-[30vh] max-w-full items-center justify-center overflow-hidden sm:max-h-[32vh]">
        <img
          src={photo.src}
...
function PhotoPage({ photos, pageIndex }: { photos: Photo[]; pageIndex: number }) {
  ...
  return (
    <section className="paper-grain relative min-h-[52vh] flex-1 overflow-hidden rounded-sm bg-cardstock px-3 py-8 shadow-[0_20px_50px_-20px_rgba(51,64,77,0.45)] sm:min-h-[58vh] sm:px-10 sm:py-14">
      ...
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 font-typewriter text-[8px] tracking-[.2em] text-ink/35">
        PAGE {pageIndex + 1}
      </div>
    </section>
  );
    ...
    <main className="min-h-screen overflow-hidden bg-[#17212b] px-3 py-6 text-[#f7f0df] sm:px-8 sm:py-10">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-7xl flex-col">
        <header className="mb-5 flex items-center justify-between sm:mb-8">
          <Link href="/" className="inline-flex items-center gap-2 font-typewriter text-xs text-white/70 transition hover:text-white sm:text-sm">
        ...
        <div className="flex flex-1 items-center justify-center">
          ...
          {!isLoading && !error && !hasPhotos && (
            <div className="text-center">
              ...
              <Link href="/" className="mt-5 inline-flex rounded-full bg-white/10 px-5 py-2 font-display text-lg hover:bg-white/20">
                Add photos
              </Link>
            </div>
          )}
            ...
            <div className="w-full max-w-6xl">
              <div className="relative mx-auto flex max-w-6xl overflow-hidden rounded-lg bg-[#d8c9a9] shadow-[0_35px_90px_-25px_rgba(0,0,0,.7)] [perspective:1800px]">
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  >
                    ...
                    <div className="h-px w-full bg-black/10 sm:h-auto sm:w-px" />
              ...
              <div className="mt-5 flex items-center justify-center gap-4 sm:mt-7 sm:gap-7">
                <button
                  ...
                  className="flex h-11 w-11 items-center justify-center rounded-full bg-white/10 transition hover:bg-white/20 disabled:opacity-20 sm:h-12 sm:w-12"
                >
                  <ChevronLeft />
                </button>
                <span className="font-typewriter text-[9px] tracking-[.18em] text-white/55 sm:text-xs">
              ...
              <p className="mt-3 text-center font-typewriter text-[9px] text-white/35 sm:text-xs">
                Photos are always shown in full • each page holds 3–5 photos when possible • double-spread photos count as one
              </p>
            </div>
          )}
        </div>
